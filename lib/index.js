...
app.use(express_session_1.default({
    secret: "keyboard vcaption webrtc",
    resave: true,
...
passport_1.default.use(new passport_http_bearer_1.default.Strategy(function (token, done) {
    winston_1.logger.debug("Passport Token: " + token);
    Token.findByToken(token, function (err, tokenFromDb) {
        ...
        Agent.findByAgentId(tokenFromDb.agentId, false, function (err, agent) {
            if (err) {
                var responseObj = new models_1.ResponseObj(401, "Unauthorized!", undefined);
                return done(err, false, responseObj.toJsonString());
            }
            ...
            return done(null, agent, { scope: "all", message: "vcaption" });
        });
    });
}));
...
server.listen(port, function () {
    winston_1.logger.debug("V3Sock app is running on port " + port);
});
